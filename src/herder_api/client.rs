...
use crate::{
    herder_api::{
        self, HerderAction, HerderActionResponse, HerderActionService, LayerError, bincode_options,
        error::NestedResultExt,
    },
    util::stdiomux::{PreambleRequestClient, StreamService, preamble_request_client},
};
...
where
    ...
    Svc: StreamService<LocalBoxStream<'static, Bytes>> + 'static,
    Svc::Error: 'static,
{
...
fn request_into_stream(action: impl HerderAction) -> impl Stream<Item = Bytes> + 'static {
    ...
    stream::once(std::future::ready(msg))
}
